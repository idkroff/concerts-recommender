...
class ConcertsGetter:
    ...
    async def extract_data_from_url(self, url: str):
        try:
            scraper_url = f"http://api.scraperapi.com?api_key={
                self.__scraper_api_token}&url={url}"
    ...
    async def get_time_for_concert(self, url: str):
        ...
        hours, minutes = map(int, soup.find(
            "span", class_="session-date__time").text.split(":"))
    ...
    async def find_concert_info(self, url: str, artist: Artist):
        ...
        try:
            ...
            all_concerts_divs = soup.findAll(
                "div", class_="person-schedule-item")
        ...
        for i in range(min(len(all_concerts_in_json), len(all_concerts_divs))):
            ...
            try:
                ...
                year, month, day = map(
                    int, concert_data.get("startDate").split("-"))
                ...
                concert_info.city = all_concerts_divs[i].find(
        ...
        try:
            ...
            for i in range(len(artist_translit)):
                if not artist_translit[i].isalnum():
                    artist_translit = artist_translit.replace(
                        artist_translit[i], "-")
            ...
            for i in range(len(artist_translit)):
                if artist_translit_temp[i] == "j":
                    artist_translit_temp = artist_translit_temp[:i] + \
                        "i" + artist_translit_temp[i + 1:]
                elif artist_translit_temp[i] == "i":
                    artist_translit_temp = artist_translit_temp[:i] + \
                        "j" + artist_translit_temp[i + 1:]
    ...
    async def extract_concerts(self, artists: List[Artist]):
        tasks = [ConcertsGetter.make_url(artist) for artist in artists]
